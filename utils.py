...
def get_upload_path_for_achievement(instance, filename):
	"""
	Генерирует уникальное имя файла для ачивки и возвращает путь для сохранения.
	"""
	ext = filename.split('.')[-1].lower()
	if ext not in ['jpg', 'png', 'jpeg', 'svg']:
		raise ValueError('Недопустимое расширение файла. Разрешены только jpg, jpeg, png, svg.')
	filename = f"{uuid.uuid4().hex}.{ext}"
	path = os.path.join(
		'achievement',  # имя папки для сохранения файлов
		filename  # имя файла
	)
	return path
